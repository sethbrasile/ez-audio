 ...
 */
export default function unmute(context, allowBackgroundPlayback = false, forceIOSBehavior = false) {
  ...
  function addEventListeners(target, events, handler, capture, passive) {
    for (let i = 0; i < events.length; ++i)
      target.addEventListener(events[i], handler, { capture, passive })
  }
  function removeEventListeners(target, events, handler, capture, passive) {
    for (let i = 0; i < events.length; ++i)
      target.removeEventListener(events[i], handler, { capture, passive })
  }
