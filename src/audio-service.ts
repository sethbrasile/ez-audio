...
export default class AudioService {
  ...
  /**
   * An AudioContext instance from the Web Audio API.
   *
   * @public
   ...
   * @type {map}
   */
  _sounds: Map<string, Sound> = new Map()
  ...
  //  */
  // stopAll(type = 'tracks') {
  //   for (const sound of this.get(`_${type}`).values()) {
  //     sound.stop()
  //   }
  // }
  ...
  //  */
  // pauseAll() {
  //   for (const sound of this._tracks.values()) {
  //     sound.pause()
  //   }
  // }
   ...
   */
  removeFromRegister(type: string, name: string) {
   ...
   */
  _createSoundFor(type: string, props: { name: string, startOffset?: number, audioBuffer: AudioBuffer }) {
    switch (type) {
      // case 'track':
      ...
      default:
        return new Sound(this.audioContext, props)
    }
  }
   ...
   * 'track', or 'beatTrack'.
   *
   * @return {promise|Sound|Track|BeatTrack|void} Returns a promise which resolves
   * to an instance of a Sound, Track, or BeatTrack
   */
  async _load(name: string, src: string, type: string): Promise<Sound | void> {
    const audioContext = this.audioContext
    const register = this._getRegisterFor(type)

    if (register.has(name)) {
      return Promise.resolve(register.get(name))
    }
    ...
    try {
      ...
      const sound = this._createSoundFor(type, { audioBuffer: decodedAudio, name })
  ...
  //  */
  // _loadFont(instrumentName: string, src) {
  ...
  //     )
  //     return Promise.resolve(fontsRegister.get(instrumentName))
  //   }
  ...
  //  */
  // _loadBeatTrack(name: string, srcArray) {
  //   const audioContext = this.audioContext
  ...
  //  */
  // _createSoundsArray(name: string, srcArray) {
  ...
  //  */
  // _extractDecodedKeyValuePairs(notes: string[]) {
  ...
  //  */
  // _createNoteObjectsForFont(audioData: [string, AudioBuffer][], instrumentName: string) {
  //   const audioContext = this.audioContext
  //   const fontsRegister = this._getRegisterFor('font')
